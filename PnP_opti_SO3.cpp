//
// Created by yitaowei on 11/16/22.
//
//
...
Eigen::Vector3d Translat(-0.044, 0, -0.19);
...
class CalibVertex : public g2o::BaseVertex<3, Sophus::SO3d> {
public:
    ...
    virtual void oplusImpl(const double *update_) {
        Eigen::Map<const Eigen::Matrix<double, 3, 1>> update(update_);
        setEstimate(Sophus::SO3d::exp(update) * estimate());
    }
};
...
public:
    ...
    virtual void computeError() override
    {
        ...
        Eigen::Vector3d dstpt = v->estimate() * orgpt + Translat;
    ...
    /*virtual void linearizeOplus() override
    {
        ...
        // Total Jacobin
        _jacobianOplusXi = 1.0 * J_pixeltoT;
    };*/
...
int main() {
    ...
    ifstream ifs;
    ifs.open("../theta_back/3D_points.txt");
    assert(ifs.is_open());
    string buff;
    int i = 0;
    ...
    {
        cout << pcd_points[i][0] << " " << pcd_points[i][1] << " " << pcd_points[i][2] << endl;
    }*/

    // load pixel value
    VecVector2d pixel_points;
    ifstream ifs_pixel;
    ifs_pixel.open("../theta_back/2D_pixels.txt");
    ...
    typedef g2o::BlockSolver<g2o::BlockSolverTraits<3, 1>> DirectBlock;  // 求解的向量是6＊1的
    ...
    vertex->setId(0);
    vertex->setEstimate(Sophus::SO3d(
            Eigen::Quaterniond(1, 0, 0, 0)));
    ...
    Sophus::SO3d Rcw = vertex->estimate();
    cout << Rcw.matrix() << endl;
    cout << Rcw.matrix().eulerAngles(2,1,0) << endl; // Z, Y, X

    Sophus::SE3d dest_calib(Rcw,Translat);

    // Calculate the reprojection error
    double cost = 0;
    for (size_t j = 0; j < 15; j++)
    {
        Eigen::Matrix<double, 3, 1> dst_pt = dest_calib * pcd_points[j] ;
    ...
    /*Mat ori_img = imread("../calib_image/014.jpg", 1);
    ...
    cv::waitKey();
    cv::imwrite("009_reprojection.jpg", ori_img);*/
